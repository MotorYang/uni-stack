...
VALUES
    (@dept_root := UUID(), '0', '0', '总部', 1, '系统', 0, NOW(6));
...
VALUES
    (@role_admin := UUID(), '超级管理员', 'ADMIN', 1, 0, '系统最高权限', NOW(6));
...
VALUES
    (@m_workbench := UUID(), '0', '工作台', 'M', '/workbench', 'DesktopOutline', 1, 0, NOW(6));

INSERT INTO sys_menu
(id, parent_id, menu_name, menu_type, path, component, icon, sort, status, create_time)
VALUES
    (UUID(), @m_workbench, '仪表盘', 'C',
     '/workbench/dashboard',
     'workbench/dashboard/index',
     'SpeedometerOutline', 1, 0, NOW(6)),

    (UUID(), @m_workbench, '服务监控', 'C',
     '/workbench/monitor',
...
(id, parent_id, menu_name, menu_type, path, icon, sort, status, create_time)
VALUES
    (@m_security := UUID(), '0', '安全治理', 'M', '/security', 'ShieldCheckmarkOutline', 2, 0, NOW(6));

INSERT INTO sys_menu
(id, parent_id, menu_name, menu_type, path, component, icon, sort, status, create_time)
VALUES
    (UUID(), @m_security, '用户管理', 'C',
     '/security/user',
     'security/user/index',
     'PersonOutline', 1, 0, NOW(6)),

    (UUID(), @m_security, '角色管理', 'C',
     '/security/role',
     'security/role/index',
     'PeopleOutline', 2, 0, NOW(6)),

    (UUID(), @m_security, '菜单配置', 'C',
     '/security/menu',
     'security/menu/index',
     'MenuOutline', 3, 0, NOW(6)),

    (UUID(), @m_security, '资源管理', 'C',
     '/security/resource',
...
(id, parent_id, menu_name, menu_type, path, icon, sort, status, create_time)
VALUES
    (@m_infra := UUID(), '0', '基础设施', 'M', '/infrastructure', 'ConstructOutline', 3, 0, NOW(6));
...
(id, parent_id, menu_name, menu_type, path, icon, sort, status, create_time)
VALUES
    (@m_api := UUID(), '0', 'API服务', 'M', '/api-service', 'ServerOutline', 4, 0, NOW(6));
...
(id, parent_id, menu_name, menu_type, path, icon, sort, status, create_time)
VALUES
    (@m_log := UUID(), '0', '日志管理', 'M', '/log-service', 'ReceiptOutline', 5, 0, NOW(6));
